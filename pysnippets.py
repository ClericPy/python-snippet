...
def init_logger(log_name=__file__, handlers=None, level=1, formatter=None, formatter_str=None):
    '''handlers is a list of ['path',level]
handlers = [['',13],['','DEBUG'],['','info'],['','notSet']]
# example:
def test():
...
help:
custom formatter:
formatter_str like: '%(levelname)-8s %(asctime)s [%(process)d:%(threadName)s] %(name)s(%(funcName)s): %(message)s'
